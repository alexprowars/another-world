{% if user.tutorial == 3 and object.item.id == 817 %}
	{% set object.item.price = 0 %}
{% endif %}
{% set t_price = (object.item.price * 0.9)|round(2) %}
{% set vip = (object.item.price * 67)|round(2) %}
{% set min = object.item.getMinDemands() %}
{% set add = object.item.getBounus() %}
<div class="shop-item">
	<table class='table item'>
		<tr>
			<td align="center" width="40%" style="vertical-align: middle">
				...
				{% elseif type == 2 %}
					...
					{% if user.proff == 2 %}
						<a href="{{ url('map/') }}?otdel={{ otdel }}&act=upgrade&id={{ object.shop.id }}">Увеличить урон предмета за 50 пл.<br>(мин +1 и мах +1)<br>(<i>Максимальная долговечноть -20</i>)</a>
					{% else %}
			...
			<td>
				<small>
					...
					{% endif %}
					<br>
					{% if object.item.f_price > 0 and type == 1 and user.vip == 1 %}
						VIP. цена: <b>{{ object.item.getVipPrice() }}</b> пл.<br>
					{% endif %}

					Долговечность: <b>{{ object.item.iznos }}</b></small><br>
					<small>Тип предмета: <i>{{ _text('game', 'weapon', object.item.tip) }}</i><br>
		...
		{% if object.item.magic > 0 or object.item.about != '' %}
			<tr>
				<td colspan="2">
