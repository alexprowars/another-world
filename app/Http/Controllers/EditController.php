...
class EditController extends Controller
{
    ...
    public function indexAction()
    {
		...
		{
			$message = $slot->onsetObject($this->request->get('onset', 'int'), $this->user);

			if ($message != '')
				$this->game->setRequestStatus(2);
		}
		...
		elseif ($this->request->get('do') == "wear")
		{
			...
			{
				$kit = UserSet::findFirst('id = '.$this->request->get('id', 'int').' AND user_id = '.$this->user->getId().'');
		...
		if (!$kit)
			...
			$inventory = UserSet::find('user_id = '.$this->user->getId());
		...
		if ($this->request->isAjax())
			$this->game->setRequestMessage($message);
		else
