...
class Battle extends Component
{
	...
	public function show()
	{
		...
		if (!$endbattle)
		{
			...
			while ($fighter = $fighters->fetch())
			{
				...
				if ($fighter['rank'] == 60 && $fighter['EndRound'] == 0)
				{
					...
					if ($user_opp)
					{
						$enemy = User::findFirst($fighter['id']);
		...
		if ($timeout > 0 && $this->user->hp_now > 0 && !$endbattle && isset($victims[$random]))
		{
			 ...
			 * @var User $enemy
			 */
			$enemy = User::findFirst($victims[$random]);
		...
		foreach ($round as $user)
		{
			foreach ($round as $enemy)
			{
				if ($user['AttackerFighter'] == $enemy['DefenderFighter'])
				{
					 ...
					 * @var User $tmp
					 */
					$tmp = User::findFirst($user['AttackerFighter']);
					 ...
					 * @var User $tmp
					 */
					$tmp = User::findFirst($enemy['AttackerFighter']);
					$this->updateUserParams($tmp, 2);
	...
	private function updateUserParams (User &$user, $userType = 1)
	{
		$slot = $user->getSlot();
	...
	private function getExp (User $user)
	{
		global $addexp;
