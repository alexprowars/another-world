...
<div class="actions">
	<div class="text-left">В игре: {{ users|length }} | <a href="?show=<{{ show == 1 ? 2 : 1 }}">{{ _text('game', 'rooms', user.room) }}</a> ({{ show == 1 ? 'мир' : 'комната' }})</div>
...
{% for pl in users %}
	<div class="people">
		...
		{% endif %}
		{% if pl['rank'] != 0 %}
			<IMG SRC="/images/rank/{{ pl['rank'] }}.gif" title="{{ _text('game', 'rank', pl['rank']) }}" width="12" height="15">
		{% endif %}
		...
		<div class="pull-xs-right">
			{% if pl['status'] != 999 %}
				 <img src="/images/chat/status{{ pl['status'] }}.gif" title="{{ _text('game', 'status', pl['status']) }}" width="15" height="15">
			{% endif %}
			{% if pl['proff'] != 0 %}
				 <img src="/images/guild/{{ pl['proff'] }}.gif" title="{{ _text('game', 'proffessions', pl['proff']) }}">
			{% endif %}
			{% if pl['silence'] > time() %}
