...
use Sky\Core\Assets\Filters\CssImports;
use Sky\Core\Assets\Filters\Cssmin;
use Sky\Core\Assets\Filters\Jsmin;
use Sky\Core\Options;
use Sky\Core\Assets\Phalcon as AssetManager;
use Phalcon\Tag;
...
class Manager
{
	...
	public function outputCss ($target = self::DEFAULT_COLLECTION_CSS)
	{
		...
		$collection->join((Options::get('assets_join_css', 0) == 1));
		...
		if ($collection->getJoin())
		{
			...
			if (Options::get('assets_minify_css', 0) == 1)
	...
	public function outputJs ($target = self::DEFAULT_COLLECTION_JS)
	{
		...
		$collection->join((Options::get('assets_join_js', 0) == 1));
		...
		if ($collection->getJoin())
		{
			...
			if (Options::get('assets_minify_js', 0) == 1)
