...
class Slots extends Model
{
	...
	{
		$this->hasOne('user_id', 'App\Models\Users', 'id');
	}
	...
	public function onsetObject ($itemId)
	{
		/**
		...
		if ($object !== false)
		{
			...
			)
			{
				...
				if ($slot != 0)
				{
					$db->updateAsDict($object->getSource(), ['onset' => $slot], "id = ".$object->id);
					$db->updateAsDict($this->getSource(), ['i'.$slot => $object->id], "id = ".$this->id);
