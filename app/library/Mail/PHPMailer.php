...
class PHPMailer
{
    ...
    protected function addAnAddress($kind, $address, $name = '')
    {
        if (!preg_match('/^(to|cc|bcc|Reply-To)$/', $kind)) {
            ...
            if ($this->exceptions) {
                throw new MailException('Invalid recipient array: ' . $kind);
            }
            return false;
        }
        ...
        if (!$this->validateAddress($address)) {
            ...
            if ($this->exceptions) {
                throw new MailException($this->lang('invalid_address') . ': ' . $address);
            }
            return false;
        }
        if ($kind != 'Reply-To') {
    ...
    public function preSend()
    {
        try {
            ...
            if ((count($this->to) + count($this->cc) + count($this->bcc)) < 1) {
                throw new MailException($this->lang('provide_address'), self::STOP_CRITICAL);
            }
            ...
            if (!$this->AllowEmpty and empty($this->Body)) {
                throw new MailException($this->lang('empty_message'), self::STOP_CRITICAL);
            }
        ...
        if ($this->SingleTo === true) {
            foreach ($this->SingleToArray as $val) {
                if (!@$mail = popen($sendmail, 'w')) {
                    throw new MailException($this->lang('execute') . $this->Sendmail, self::STOP_CRITICAL);
                }
                fputs($mail, "To: " . $val . "\n");
                fputs($mail, $header);
                fputs($mail, $body);
    ...
    protected function mailSend($header, $body)
    {
        ...
        if (!$rt) {
            throw new MailException($this->lang('instantiate'), self::STOP_CRITICAL);
        }
        return true;
    }
    ...
    protected function smtpSend($header, $body)
    {
        ...
        if (!$this->smtpConnect()) {
            throw new MailException($this->lang('smtp_connect_failed'), self::STOP_CRITICAL);
        }
        $smtp_from = ($this->Sender == '') ? $this->From : $this->Sender;
        if (!$this->smtp->mail($smtp_from)) {
            $this->setError($this->lang('from_failed') . $smtp_from . ' : ' . implode(',', $this->smtp->getError()));
            throw new MailException($this->ErrorInfo, self::STOP_CRITICAL);
        }
        ...
        if (!$this->smtp->data($header . $body)) {
            throw new MailException($this->lang('data_not_accepted'), self::STOP_CRITICAL);
        }
                ...
                try {
                    ...
                    if ($tls) {
                        if (!$this->smtp->startTLS()) {
                            throw new MailException($this->lang('connect_host'));
                        }
                    ...
                    if ($this->SMTPAuth) {
                        if (!$this->smtp->authenticate(
                            ...
                            throw new MailException($this->lang('authenticate'));
                        }
                    }
                    return true;
                } catch (MailException $e) {
        ...
        if ($this->isError()) {
            ...
            try {
                if (!defined('PKCS7_TEXT')) {
                    throw new MailException($this->lang('signing') . ' OpenSSL extension missing.');
                }
                ...
                if (@openssl_pkcs7_sign(
                    ...
                    @unlink($file);
                    @unlink($signed);
                    throw new MailException($this->lang('signing') . openssl_error_string());
                }
            } catch (MailException $e) {
                $body = '';
                if ($this->exceptions) {
    ...
    public function addAttachment($path, $name = '', $encoding = 'base64', $type = '', $disposition = 'attachment')
    {
        try {
            if (!@is_file($path)) {
                throw new MailException($this->lang('file_access') . $path, self::STOP_CONTINUE);
            }
    ...
    protected function encodeFile($path, $encoding = 'base64')
    {
        try {
            if (!is_readable($path)) {
                throw new MailException($this->lang('file_open') . $path, self::STOP_CONTINUE);
            }
    ...
    public function set($name, $value = '')
    {
        try {
            if (isset($this->$name)) {
                ...
                throw new MailException($this->lang('variable_set') . $name, self::STOP_CRITICAL);
            }
        } catch (Exception $e) {
            $this->setError($e->getMessage());
